...
- 设置新增“AI发送记录”，本机保存每条 AI 生成并交给发信链的时间、私聊/群聊、目标、完整正文、触发方式和发送结果。
- 自动回复与测试发话都进入审计记录；支持逐条查看、复制单条、复制全部和二次确认后清空。
- 审计标记只存在插件日志中，对方收到的聊天正文不会添加“AI自动发送”或任何其他 AI 标记。
- 最多保存最近 500 条，数据仅写入抖音 App 本机偏好存储，不上传到 GitHub 或 DeepSeek。
